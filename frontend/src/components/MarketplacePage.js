    ...
    filteredAssets.forEach(asset => {
      const marker = new window.google.maps.Marker({
        ...
        icon: {
          ...
          scale: 8,
          fillColor: asset.status === 'Live' ? '#dc2626' : '#10b981',
          fillOpacity: 1,
          strokeColor: '#ffffff',
          strokeWeight: 2
        }
  ...
  const handleOfferSubmit = async (redirectToDashboard = true) => {
    ...
    try {
      ...
      const offerRequestData = {
        ...
        asset_start_date: offerDetails.tentativeStartDate ? 
          `${offerDetails.tentativeStartDate.getFullYear()}-${String(offerDetails.tentativeStartDate.getMonth() + 1).padStart(2, '0')}-${String(offerDetails.tentativeStartDate.getDate()).padStart(2, '0')}` 
          : null,
        asset_expiration_date: offerDetails.assetExpirationDate ? 
          `${offerDetails.assetExpirationDate.getFullYear()}-${String(offerDetails.assetExpirationDate.getMonth() + 1).padStart(2, '0')}-${String(offerDetails.assetExpirationDate.getDate()).padStart(2, '0')}` 
          : null,
                        ...
                        <Popover>
                          ...
                          <PopoverContent className="w-auto p-0">
                            <CalendarComponent
                              ...
                              disabled={(date) => {
                                // For live assets or assets waiting for go live with asset expiry date, disable dates before expiry date + 1 day
                                if (selectedAssetForOffer && (selectedAssetForOffer.status === 'Live' || selectedAssetForOffer.waiting_for_go_live) && 
                                  ...
                                  const nextAvailableDate = new Date(selectedAssetForOffer.asset_expiry_date);
      ...
      {selectedAsset && (
        <Dialog open={!!selectedAsset} onOpenChange={() => {
          ...
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center space-x-2">
                <div className={`w-3 h-3 rounded-full ${statusColors[selectedAsset.status === 'Pending Offer' ? 'Available' : selectedAsset.status]}`} />
              ...
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 rounded-lg p-4">
                  ...
                  <div className="flex items-center space-x-2 mb-3">
                    <Badge 
                      variant={selectedAsset.status === 'Available' || selectedAsset.status === 'Pending Offer' ? 'success' : selectedAsset.status === 'Booked' || selectedAsset.status === 'Live' ? 'secondary' : 'default'}
                      className={
                        ...
                        selectedAsset.waiting_for_go_live ? 'bg-orange-100 text-orange-800' :
                  ...
                  {/* Show Next Available Date for assets with PO Uploaded status OR Live assets with expiry date */}
                  {((selectedAsset.waiting_for_go_live && selectedAsset.asset_expiry_date) || 
                    (selectedAsset.status === 'Live' && selectedAsset.asset_expiry_date)) && (
