...
const BeatSpace = () => {
  ...
  const createInfoWindowContent = (asset) => {
    ...
    return `
      <div style="max-width: 300px; font-family: system-ui;">
        ...
        <p style="margin: 8px 0; font-weight: 600;">From AED ${pricing.toLocaleString()}</p>
  ...
  const applyFilters = () => {
    ...
    if (filters.minPrice || filters.maxPrice) {
      filtered = filtered.filter(asset => {
        const price = asset.pricing[filters.duration] || Object.values(asset.pricing)[0];
  ...
  const calculateCampaignTotal = () => {
    return campaign.reduce((total, asset) => {
      return total + (asset.pricing[filters.duration] || Object.values(asset.pricing)[0]);
    }, 0);
  };
  ...
  const submitBestOfferRequest = async () => {
    try {
      ...
      const campaignData = {
        name: offerDetails.campaignName,
        buyer_id: 'buyer_demo_001',
        buyer_name: 'Demo Buyer',
        asset_ids: campaign.map(asset => asset.id),
        description: offerDetails.notes,
        budget: parseFloat(offerDetails.budget || calculateCampaignTotal()),
        notes: offerDetails.notes
      };

      const campaignResponse = await axios.post(`${API}/campaigns`, campaignData);
      const campaignId = campaignResponse.data.id;

      // Submit best offer request
      const offerRequest = {
        campaign_id: campaignId,
        asset_requirements: campaign.reduce((acc, asset) => {
          acc[asset.id] = {
            duration: offerDetails.duration,
            services: ['setup', 'monitoring'],
            notes: `Selected for ${offerDetails.campaignName}`
          };
          return acc;
        }, {}),
        timeline: 'Standard timeline',
        special_requirements: offerDetails.notes
      };

      await axios.post(`${API}/campaigns/${campaignId}/request-offer`, offerRequest);
      
      alert('Best Offer Request submitted successfully! You will be notified when our team has prepared your customized offer.');
                  ...
                  <DialogContent className="max-w-2xl">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          <label className="block text-sm font-medium mb-2">Estimated Budget (AED)</label>
                          <Input
                            type="number"
                      ...
                      <div className="bg-gray-50 p-4 rounded-lg">
                        ...
                        <div className="space-y-2 max-h-40 overflow-y-auto">
                          {campaign.map(asset => (
                            <div key={asset.id} className="flex justify-between items-center text-sm bg-white p-2 rounded">
                              ...
                              <span className="font-medium">AED {(asset.pricing[offerDetails.duration] || Object.values(asset.pricing)[0]).toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                        ...
                        <div className="flex justify-between items-center font-semibold">
                          ...
                          <span>AED {calculateCampaignTotal().toLocaleString()}</span>
                        </div>
                      </div>
